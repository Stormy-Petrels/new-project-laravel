...
class SignUpController extends Controller
{
    ...
    public function signUpNew(Request $req)
    {
        ...
        $newUser = new User(Role::Patient, $req->input('email'), $req->input('password'), $req->input('name'), $req->input('address'), $req->input('phone'));
        $newPatient = new Patient($newUser->getId());
        ...
        $requestPatient = new PatientRepository();
        $patientId = $requestPatient->findByEmail($req->email);
        ...
        //     )
        // ], 200);
        return redirect()->route('index')->with('new_user', $newUser);
    }
}